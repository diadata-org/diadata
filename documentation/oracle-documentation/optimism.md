---
title: DIA Data
description: How to use request data from a DIA Oracle in your Optimism Dapp using smart contracts
---

# DIA Data Oracles on Optimism
...
DApp developers who want to leverage DIA oracles can access the published data on Optimism.
...
| USDT          |   USDT/USD                  |  Crypto Price                       |
| DAI           |   DAI/USD                   |  Crypto price                       |
| DIA Token     |   DIA/USD                   |  Crypto price                       |

To retrieve data, query the oracle read function `getValue()` for an asset as listed in column "Data Field Query".
...
They are denominated in a fix comma format with 8 decimals, so you need to divide them by 1e8 to retrieve the actual value in USD.
...
You can interact with our contract in the [blockchain explorer](https://optimistic.etherscan.io/address/0x7a924305e8c00757db6131d665e5d559ead9f7cd), where you can query for any supported asset.

The contract takes the symbol name of the asset as input, e.g., `BTC/USD` and returns a tuple of the timestamp of the last update and the actual value.
...
On Optimism, you can initialize the oracle address in your wrapper contract by calling `setOracleAddress()` with the address of our deployed demo contract `0x7A924305e8C00757db6131D665e5D559EAD9f7Cd`.
...
| :------------ | :------------------------------------------: | :----------: |
| Optimism      | `0x7A924305e8C00757db6131D665e5D559EAD9f7Cd` |    1/hour    |
