...
/* TODO REMOVE
func GetLastHash(topic int) string {
	for {
		var offset = ReadOffsetWithRetryOnError(topic)
		offset--
		if offset < 0 {
			return FirstBlockHash

		} else {
			element, err := GetElements(topic, offset, 1)
			if err != nil {
				log.Println("error fetching last hash <", err, "> ")
				time.Sleep(2 * time.Second)
			} else {
				e := element[0].(KafkaMessageWithAHash)
				return e.Hash()
			}
		}
	}
