...
func (rdb *RelDB) SearchNFT(searchstring string) (nfts []dia.NFT, err error) {
	...
	query = `SELECT DISTINCT ON (address) address, symbol, name, contract_type, category, n.creator_address, n.uri, n.attributes from nftclass nc INNER JOIN nft n ON  n.nftclass_id=nc.nftclass_id WHERE symbol ILIKE '%` + searchstring + `%' or address ILIKE' %` + searchstring + `%  or name ILIKE' %` + searchstring + `%';`
