...
func NewMaverickScraper(exchange dia.Exchange, relDB *models.RelDB, datastore *models.DB) (s *MaverickScraper) {
	...
	case dia.MaverickExchange:
		s = makeMaverickPoolScraper(exchange, pathToPools, exchange.RestAPI, relDB, datastore, defaultWaitMillis, factoryContractAddressDeploymentBlockEth)
...
func makeMaverickPoolScraper(exchange dia.Exchange, pathToPools string, restDial string, relDB *models.RelDB, datastore *models.DB, waitMilliseconds string, factoryContractDeploymentBlock uint64) *MaverickScraper {
	var (
		restClient  *ethclient.Client
		err         error
...
func (s *MaverickScraper) getPoolByAddress(pairAddress common.Address) (pool dia.Pool, err error) {
	...
	amount0, _ := new(big.Float).Quo(big.NewFloat(0).SetInt(balanceA), new(big.Float).SetFloat64(math.Pow10(18))).Float64()
	amount1, _ := new(big.Float).Quo(big.NewFloat(0).SetInt(balanceB), new(big.Float).SetFloat64(math.Pow10(18))).Float64()
