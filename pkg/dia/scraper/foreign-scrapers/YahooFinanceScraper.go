	...
	for k := range scraper.currenciesMap {
		...
		for _, result := range chartDataRes {
			...
			if (len(symbol) == 8 || len(symbol) == 5) && symbol[len(symbol)-2:] == "=X" {
				...
				// Iterate through data starting at most recent timestamp with non-null price
				// in order to store this one and continue with next pair.
				numData := len(result.Timestamp)
				for i := range result.Timestamp {
					quoteDateTime := time.Unix(int64(result.Timestamp[numData-i-1]), 0)
					quotePrice := result.Indicators.Quote[0].Close[numData-i-1]
					if quotePrice == 0 {
						continue
					}
