...
type BinanceScraper struct {
	...
	pendingSubs       chan dia.ExchangePair
}
...
func NewBinanceScraper(apiKey string, secretKey string, exchange dia.Exchange, scraperName string, scrape bool, relDB *models.RelDB) *BinanceScraper {
	...
	s := &BinanceScraper{
		...
		pendingSubs:     make(chan dia.ExchangePair, 100),
	}
...
func (s *BinanceScraper) processSubscriptions() {
	for pair := range s.pendingSubs {
		ctx, cancel := context.WithTimeout(context.Background(), 5*time.Second)
		if err := s.rateLimiter.Wait(ctx); err != nil {
			log.Error("Rate limit exceeded:", err)
			cancel()
			continue
		}
		cancel()
...
func (s *BinanceScraper) ScrapePair(pair dia.ExchangePair) (PairScraper, error) {
	...
	/*
		Collect all the pairs, then iterate through each pair again and use a rate limiter
		in the processSubscriptions() function to ensure that no more than 5 requests are sent per second
		(as required by Binance).
	*/
	select {
	case s.pendingSubs <- pair:
		log.Info("Subscription queued:", pair.ForeignName)
	default:
		return ps, errors.New("subscription queue overflow")
	}
