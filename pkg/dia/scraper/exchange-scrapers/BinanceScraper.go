...
func (scraper *BinanceScraper) subscribe(pair dia.ExchangePair, subscribe bool) error {
	...
	pairTicker := strings.ToLower(pair.Symbol + pair.ForeignName[len(pair.Symbol):])
	...
	if err := scraper.wsClient.WriteJSON(subscribeMessage); err != nil {
		log.Errorf("Failed to send subscription request: %w", err)
		return err
	}
...
func (s *BinanceScraper) parseWSResponse(message binanceWSResponse) {
	...
	tradeTime := time.Unix(message.Timestamp/1000, (message.Timestamp%1000)*int64(time.Millisecond))
...
func (s *BinanceScraper) ScrapePair(pair dia.ExchangePair) (PairScraper, error) {
	...
	if err != nil {
		log.Errorf("Failed to subscribe to pair %s: %v", pair.Symbol, err)
	} else {
		log.Infof("Successfully subscribe to pair %s", pair.Symbol)
	}
