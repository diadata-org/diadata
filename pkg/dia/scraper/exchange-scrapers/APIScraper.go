...
func NewAPIScraper(exchange string, scrape bool, key string, secret string, relDB *models.RelDB) APIScraper {
	...
	case dia.UniswapExchangeArbitrum:
		return NewUniswapScraper(Exchanges[dia.UniswapExchangeArbitrum], scrape, relDB)
	...
	case dia.BeetsV2ExchangeSonic:
		return NewBalancerV2Scraper(Exchanges[dia.BeetsV2ExchangeSonic], scrape, relDB)
	...
	case dia.ShadowV2Exchange:
		return NewUniswapScraper(Exchanges[dia.ShadowV2Exchange], scrape, relDB)
	case dia.ShadowV3Exchange:
		return NewUniswapV3Scraper(Exchanges[dia.ShadowV3Exchange], scrape, relDB)
		// case dia.FinageForex:
		// 	return NewFinageForexScraper(Exchanges[dia.FinageForex], scrape, relDB, key, secret)
