...
func main() {
	...
	var secretsFile = flag.String("secretsFile", "/run/secrets/oracle_keys_utopiansworld_aurora", "File with wallet secrets")
	var blockchainNode = flag.String("blockchainNode", "https://andromeda.metis.io/?owner=1088", "Node address for blockchain connection")
	var sleepSeconds = flag.Int("sleepSeconds", 10, "Number of seconds to sleep between calls")
	var frequencySeconds = flag.Int("frequencySeconds", 240, "Number of seconds to sleep between checking oracle runs")
	var chainId = flag.Int64("chainId", 1088, "Chain-ID of the network to connect to")
	flag.Parse()
	...
	//METIS
	symbols := []string{"0x9E32b13ce7f2E80A01932B42553652E053D6ed8e"}
		...
		for {
			...
			case <-ticker.C:
				for i, symbol := range symbols {
					...
					err = periodicOracleUpdateHelper(auth, contract, conn, blockchain, symbol)
					if err != nil {
						log.Println(err)
					}
...
func getQuotationFromDia(symbol string) (*models.Quotation, error) {
	response, err := http.Get("https://rest.diadata.org/v1/quotation/" + strings.ToUpper(symbol))
	if err != nil {
		return nil, err
	}
