...
func retrieveAssetPrice(asset Asset, useGql bool, gqlWindowSize int, gqlMethodology string, gqlLiquidityParameters GqlParameters, dataNode string, oracleDecimals int) (float64, error) {
	var err error
	var price float64
	...
	// Check if the asset is satUSD+
	if strings.ToLower(strings.TrimSpace(asset.blockchain)) == "erc4646" {
		// Get conversion rate in ERC4646 Vault
		conversionRate := getConvertToSharesFrom4646Vault(dataNode, asset.address, oracleDecimals)
		return float64(conversionRate) / math.Pow10(oracleDecimals), nil
	}
...
func getConvertToSharesFrom4646Vault(dataNode string, vaultAddress string, decimals int) int64 {
	...
	vaultConn, err := vault.NewErc4646Vault(common.HexToAddress(vaultAddress), dataConn)
	if err != nil {
		log.Fatalf("Failed to instantiate Erc4646Vault: %v", err)
	}
	convertValue := int64(math.Pow10(decimals))
	convertedShares, err := vaultConn.ConvertToShares(&bind.CallOpts{}, big.NewInt(convertValue))
	if err != nil {
		log.Fatalf("Failed to call convertToShares in smart contract: %v", err)
	}
	return convertedShares.Int64()
}
