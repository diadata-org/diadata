			...
			if mandatoryFrequencySeconds > 0 {
				...
				case <-ticker.C:
					...
					for _, asset := range assets {
						newAssetPrice, err := retrieveAssetPrice(asset, useGql, gqlWindowSize, gqlMethodology, asset.gqlParams, dataNode, canaInstanceAddress, oracleDecimals)
						if err != nil {
							log.Println(err)
							continue
						}
...
func retrieveAssetPrice(asset Asset, useGql bool, gqlWindowSize int, gqlMethodology string, gqlLiquidityParameters GqlParameters, dataNode string, canaInstanceAddress string, oracleDecimals int) (float64, error) {
	var err error
	var price float64
	...
	// Check if the asset is Cana for NAV
	if strings.ToLower(strings.TrimSpace(asset.blockchain)) == "nav" && strings.ToLower(strings.TrimSpace(asset.address)) == "cana" {
		navprice, err := getCanaNavFromChain(dataNode, canaInstanceAddress)
		if err != nil {
...
func getCanaNavFromChain(dataNode string, contractInstanceAddress string) (uint64, error) {
	dataConn, err := ethclient.Dial(dataNode)
	if err != nil {
		log.Printf("Failed to connect to the data client: %v", err)
		return 0, err
	}
	diaOracleConn, err := NewMain(common.HexToAddress(contractInstanceAddress), dataConn)
	if err != nil {
		log.Printf("Failed to instantiate MaseerCana instance: %v", err)
		return 0, err
	}
	navPrice, err := diaOracleConn.Navprice(&bind.CallOpts{})
	if err != nil {
		log.Printf("Failed to call navprice in smart contract: %v", err)
		return 0, err
	}
	retval := navPrice.Uint64()
	return retval, nil
}
