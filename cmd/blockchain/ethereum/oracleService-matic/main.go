...
func periodicOracleUpdateHelper(sleepSeconds int, auth *bind.TransactOpts, contract *oracleService2.DiaOracle, conn *ethclient.Client) error {
	...
	// MATIC Quotation
	rawMATICQ, err := getQuotationFromDia("MATIC")
	if err != nil {
		log.Fatalf("Failed to retrieve MATIC quotation data from DIA: %v", err)
		return err
	}
	rawMATICS, err := getSupplyFromDia("MATIC")
	if err != nil {
		log.Fatalf("Failed to retrieve MATIC supply data from DIA: %v", err)
		return err
	}
	err = updateQuotation(rawMATICQ, rawMATICS, auth, contract, conn)
	if err != nil {
		log.Fatalf("Failed to update MATIC Oracle: %v", err)
		return err
	}
	time.Sleep(time.Duration(sleepSeconds) * time.Second)
...
func updateCoin(coin models.Coin, auth *bind.TransactOpts, contract *oracleService2.DiaOracle, conn *ethclient.Client) error {
...
func getCoinDetailsFromDia(symbol string) (*models.Coin, error) {
	contents, statusCode, err := utils.GetRequest(dia.BaseUrl + "/v1/symbol/" + symbol)
	if err != nil {
		return nil, err
	}
	if statusCode != 200 {
