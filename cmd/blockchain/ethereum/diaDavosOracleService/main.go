	...
	go func() {
		for {
			if mandatoryFrequencySeconds > 0 {
				select {
				case <-ticker.C:
					// Get target rate from NY Fed API
					targetRate, err := getReferenceRateFromFed("EFFR")
					if err != nil {
						log.Println(err)
						continue
					}
				  log.Println(targetRate)
				  if targetRate == 0.0 {
				  	log.Println("Error! NY Fed API return was empty")
				  	continue
					}
					// update value
					publishedValue, err = oracleUpdateExecutor(publishedValue, targetRate, deviationPermille, auth, contract, conn, chainId)
					if err != nil {
						...
						publishedValue, err = oracleUpdateExecutor(publishedValue, targetRate, deviationPermille, auth, contractBackup, connBackup, chainId)
						if err != nil {
				...
				case <-mandatoryticker.C:
				  ...
				  if targetRate == 0.0 {
					...
					// update value
					publishedValue, err = oracleUpdateExecutor(0.0, targetRate, deviationPermille, auth, contract, conn, chainId)
					if err != nil {
						log.Printf("Failed to execute oracle update using primary connection: %v. Retrying with backup connection...", err)

						// Attempt using the backup connection
						publishedValue, err = oracleUpdateExecutor(0.0, targetRate, deviationPermille, auth, contractBackup, connBackup, chainId)
						if err != nil {
							log.Fatalf("Failed to execute oracle update using backup connection: %v", err)
						}
					}
...
func oracleUpdateExecutor(
	...
	keys = append(keys, "targetRateFrom")
...
func getReferenceRateFromFed(rateName string) (float64, error) {
	// Execute the query
	response, err := http.Get("https://markets.newyorkfed.org/api/rates/all/latest.json")
	if err != nil {
		return 0.0, err
	}
	...
	if 200 != response.StatusCode {
		return 0.0, fmt.Errorf("Error on NY Fed api with return code %d", response.StatusCode)
	}
	...
	targetRate := gjson.Get(string(contents), "refRates.#(type==\"" + rateName + "\").targetRateFrom").Float()
