...
const (
	baseUrl      = "https://reserve.denario.swiss/api"
)
			...
			if mandatoryFrequencySeconds > 0 {
				...
				case <-ticker.C:
					...
					for _, asset := range denarioAssets {
						newAssetPrice, err := retrieveDenarioPor(baseUrl, asset, authKey)
						if err != nil {
...
func retrieveDenarioPor(baseUrl string, assetName string, authKey string) (float64, error) {
  // Execute the query
	client := http.Client{}
  ...
  porValue := gjson.Get(string(contents), "amountOunces").Float()
  return porValue, nil
}
