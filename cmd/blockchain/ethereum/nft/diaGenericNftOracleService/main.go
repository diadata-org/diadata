...
func main() {
	...
	floorWindow, err := strconv.Atoi(utils.Getenv("FLOOR_WINDOW", "1800"))
	if err != nil {
		log.Fatalf("Failed to parse floorWindow: %v")
	}
	lookbackSeconds, err := strconv.Atoi(utils.Getenv("LOOKBACK_SECONDS", "86400"))
	if err != nil {
		log.Fatalf("Failed to parse lookbackSeconds: %v")
	}
		...
		for {
			...
			case <-ticker.C:
				for i, address := range addresses {
					...
					oldValue, err = periodicOracleUpdateHelper(oldValue, deviationPermille, auth, contract, conn, blockchain, address, floorWindow, lookbackSeconds)
...
func periodicOracleUpdateHelper(oldValue float64, deviationPermille int, auth *bind.TransactOpts, contract *diaOracleServiceV2.DIAOracleV2, conn *ethclient.Client, blockchain string, address string, floorWindow, lookbackSeconds int) (float64, error) {

	// Empty quotation for our request
	var err error

	// Get floorMA price
	floorMA, err := getFloorMA(blockchain, address, floorWindow, lookbackSeconds)
	if err != nil {
	...
	if (newValue > (oldValue * (1 + float64(deviationPermille)/1000))) || (newValue < (oldValue * (1 - float64(deviationPermille)/1000))) {
		...
		rawFloorMA := floorMA.Value

		err = updateFloorMA(rawFloorMA, blockchain, address, auth, contract, conn)
		if err != nil {
...
func updateFloorMA(floorMA float64, blockchain string, address string, auth *bind.TransactOpts, contract *diaOracleServiceV2.DIAOracleV2, conn *ethclient.Client) error {
	symbol := blockchain + "-" + address 
	timestamp := time.Now().Unix()
	err := updateOracle(conn, contract, auth, symbol + "-FloorMA", int64(floorMA*100000000), timestamp)
	if err != nil {
...
func getFloorMA(blockchain, address string, floorWindow, lookbackSeconds int) (FloorMA, error) {
	response, err := http.Get("https://api.diadata.org/v1/NFTFloorMA/" + blockchain + "/" + address + "?lookbackSeconds=" + strconv.Itoa(lookbackSeconds) + "&floorWindow=" + strconv.Itoa(floorWindow))
	if err != nil {
		return FloorMA{}, err
	}
