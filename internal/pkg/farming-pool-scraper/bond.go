...
import (
	...
	yieldfarm "github.com/diadata-org/diadata/internal/pkg/farming-pool-scraper/barnbridge/yieldfarm"
	yieldfarmbond "github.com/diadata-org/diadata/internal/pkg/farming-pool-scraper/barnbridge/yieldfarmbond"
	yieldfarmlp "github.com/diadata-org/diadata/internal/pkg/farming-pool-scraper/barnbridge/yieldfarmlp"
...
func (bd *BONDScraper) scrapePools() (err error) {
	...
	for _, poolDetail := range bd.getBONDPools() {
		...
		yieldfarm, err := yieldfarm.NewYieldfarmCaller(common.HexToAddress(poolDetail.VaultAddress), bd.RestClient)
		if err != nil {
			return err
		}
		yieldfarmbond, err := yieldfarmbond.NewYieldfarmbondCaller(common.HexToAddress(poolDetail.VaultAddress), bd.RestClient)
		if err != nil {
			return err
		}
		yieldfarmlp, err := yieldfarmlp.NewYieldfarmlpCaller(common.HexToAddress(poolDetail.VaultAddress), bd.RestClient)
		if err != nil {
			return err
		}
		...
		yieldfarmSize, err := yieldfarm.GetPoolSize(&bind.CallOpts{}, currentEpoch)
		if err != nil {
			return err
		}
		yieldfarmLpSize, err := yieldfarmlp.GetPoolSize(&bind.CallOpts{}, currentEpoch)
		if err != nil {
			return err
		}
		yieldfarmBondSize, err := yieldfarmbond.GetPoolSize(&bind.CallOpts{}, currentEpoch)
		if err != nil {
			return err
		}

		var totalYield = big.NewInt(0).Add(yieldfarmSize, yieldfarmLpSize)
		var priceStablePool = big.NewInt(0).Add(totalYield, yieldfarmBondSize)
...
func (bd *BONDScraper) getBONDPools() (pools []*BONDPoolDetail) {
	...
	pools = append(pools, &BONDPoolDetail{TokenName: "Staking", VaultAddress: "0xb0Fa2BeEe3Cf36a7Ac7E99B885b48538Ab364853", PoolID: "Staking"})
