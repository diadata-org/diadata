...
import (
	...
	comp "github.com/diadata-org/diadata/internal/pkg/farming-pool-scraper/curveficontracts/comp"
	susd "github.com/diadata-org/diadata/internal/pkg/farming-pool-scraper/curveficontracts/susdv2-pool"
...
func (cv *CFIPool) scrapePools() (err error) {
	...
	for _, poolDetail := range cv.getCFIPools() {
		...
		pool, err := susd.NewSusdCaller(common.HexToAddress(poolDetail.PoolAddress), cv.RestClient)
		if err != nil {
			return err
		}
		...
		for _, name := range normalPoolAndSwap {
			if name == poolDetail.PoolName {
				...
				coinsAddress0, _ := pool.Coins(&bind.CallOpts{}, big.NewInt(0))
				balance0, _ := pool.Balances(&bind.CallOpts{}, big.NewInt(0))
				...
				if poolDetail.PoolName == "3" || poolDetail.PoolName == "sbtc" || poolDetail.PoolName == "hbtc" || poolDetail.PoolName == "ren" {
					coinsAddress1, _ := pool.Coins(&bind.CallOpts{}, big.NewInt(1))
					balance1, _ := pool.Balances(&bind.CallOpts{}, big.NewInt(1))
					...
					if poolDetail.PoolName == "3" || poolDetail.PoolName == "sbtc" {
						coinsAddress2, _ := pool.Coins(&bind.CallOpts{}, big.NewInt(2))
						balance2, _ := pool.Balances(&bind.CallOpts{}, big.NewInt(2))
		...
		if poolDetail.PoolName == "Compound" || poolDetail.PoolName == "USDT" {
			...
			comp0, err := comp.NewCompCaller(common.HexToAddress(coinsAddress0.Hex()), cv.RestClient)
			if err != nil {
				return err
			}
			...
			comp1, err := comp.NewCompCaller(common.HexToAddress(coinsAddress1.Hex()), cv.RestClient)
			if err != nil {
				return err
			}
		...
		if poolDetail.PoolName == "PAX" || poolDetail.PoolName == "BUSD" || poolDetail.PoolName == "y" {
			...
			if poolDetail.PoolName == "PAX" {
				coinsAddress3, _ := pool.Coins(&bind.CallOpts{}, big.NewInt(3))
				balance3, _ := pool.Balances(&bind.CallOpts{}, big.NewInt(3))
